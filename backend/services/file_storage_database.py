...
class FileStorageDatabase:
    ...
    async def list_files_by_project(self, project_id: str, limit: int = 100, offset: int = 0) -> List[FileMetadata]:
        ...
        try:
            ...
            # Sort by upload date (newest first)
            project_files.sort(key=lambda x: x.uploaded_at, reverse=True)
    ...
    async def get_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            stats = {
                ...
                "oldest_file": min(f.uploaded_at for f in files).isoformat(),
                "newest_file": max(f.uploaded_at for f in files).isoformat()
            }
